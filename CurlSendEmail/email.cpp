...
const int ENCODED_LEN = MAX_LEN * 2 + 1;
...
void Email::setCc(const std::string& to)
{
	this->cc = to;
}
...
void Email::setSMTP_username(const std::string& username)
{
	this->smtp_user = username;
}

void Email::setSMTP_password(const std::string& password)
{
	this->smtp_password = password;
}

//eg: smtp://smtp.163.com:25   smtps://smtp.163.com:465     
void Email::setSMTP_uri(const std::string& uri)
{
	this->smtp_uri = uri;
}
...
static std::string GetFileNameFromPath(const std::string & sPath)
{
	...
	return sPath.substr(split + 1);
}

void Email::addAttachment(const std::string& filepath)
{
	FILE *fp = NULL;
	...
	if (fp) {
		...
		std::string filename = GetFileNameFromPath(filepath);// (char *)strrchr(file_path.c_str(), DIR_CHAR);

			// push the filename
		snprintf(tempBuffer, MAX_LEN, "  filename=\"%s\"\r\n", filename.c_str());

		this->attachments.push_back(std::string(tempBuffer));
		...
		while (bytesCopied < fileSize) {
			...
			int nEncodedCount = 0;
			base64_encode((const unsigned char*)buffer, bytesRead, encodedBuffer, nEncodedCount);
...
int Email::send(void) const
{
	...
	if (curl) {
		...
		curl_easy_setopt(curl, CURLOPT_URL, this->smtp_uri.c_str());
...
void Email::dump(const std::string& dumpFile) const
{
	FILE* fp = fopen(dumpFile.c_str(), "wb");
	if (fp)
	{
		for (int i = 0; i < this->email_contents.size(); i++)
		{
			fwrite(this->email_contents[i].c_str(), this->email_contents[i].length(), 1, fp);
		}
